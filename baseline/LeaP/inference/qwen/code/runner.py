...
class DirectVLLMInferPipeline:
    def __init__(self, model_path, num_gpus, gpu_memory_utilization, tensor_parallel_size, max_model_len=None):
...
LIVECODEBENCH_OUTPUT_SKIP_KEYS = {
    "index",
    "question_id",
    "question_content",
    "starter_code",
...
def load_livecodebench_data(input_file):
    path = Path(input_file)
    if not path.exists():
        raise FileNotFoundError(f"LiveCodeBench file not found: {path}")
    if path.suffix == ".jsonl":
        return load_jsonl(path), path
    with open(path, "r", encoding="utf-8") as f:
        return json.load(f), path
...
def normalize_livecodebench_item(item, idx):
    merged = dict(item)
    merged["question_id"] = merged.get("question_id", merged.get("id", f"q_{idx}"))
    merged["question_content"] = _text(
        merged.get("question_content", merged.get("question", merged.get("problem", "")))
    )
    merged["starter_code"] = _text(merged.get("starter_code", ""))
    return merged
...
def extract_leap_livecodebench_completion(text):
    raw = "" if text is None else str(text)

    assistant_marker = "<|im_start|>assistant\n"
    assistant_text = raw.rsplit(assistant_marker, 1)[1].strip() if assistant_marker in raw else raw.strip()
    assistant_text = assistant_text.replace("<|im_end|>", "").strip()

    reasoning_content = ""
    candidate = assistant_text
    end_tag = "</think>"
    if end_tag in candidate:
        reasoning_content, candidate = candidate.rsplit(end_tag, 1)
    ...
    marker_patterns = [
        ...
        r"###\s*Answer\s*:",
        r"complete Python solution in one\s+```python\s+code block\.",
    ]
    last_marker = None
    for pattern in marker_patterns:
        matches = list(re.finditer(pattern, candidate, flags=re.IGNORECASE))
        if matches and (last_marker is None or matches[-1].end() > last_marker.end()):
            last_marker = matches[-1]
    if last_marker:
        if not reasoning_content:
            reasoning_content = candidate[:last_marker.start()].replace("<think>", "").strip()
        candidate = candidate[last_marker.end():].strip()

    fence_matches = list(re.finditer(r"```(?:python)?\s*\n.*?```", candidate, flags=re.DOTALL | re.IGNORECASE))
    if fence_matches:
        match = fence_matches[-1]
        if not reasoning_content:
            reasoning_content = candidate[:match.start()].replace("<think>", "").strip()
        return match.group(0).strip(), reasoning_content

    code_start = re.search(r"(?:^|\n)(import\s+|from\s+|def\s+\w+|class\s+\w+)", candidate)
    if code_start:
        if not reasoning_content:
            reasoning_content = candidate[:code_start.start()].replace("<think>", "").strip()
        candidate = candidate[code_start.start():].strip()

    return candidate.strip(), reasoning_content


def _is_valid_completion(completion):
    text = completion.get("text") or ""
    reasoning = completion.get("reasoning_content") or ""
    if "API call failed" in text or "API调用失败" in text:
        return False
...
def load_existing_livecodebench_output(path):
    p = Path(path)
    if not p.exists():
        return {"data": None, "valid_completion_list": [], "valid_completions": 0}
...
def make_completion_records(solutions, token_counts, finish_reasons, dataset="livecodebench"):
    records = []
    for i, solution in enumerate(solutions):
        if dataset == "livecodebench":
            final_text, reasoning_content = extract_leap_livecodebench_completion(solution)
        else:
...
def save_livecodebench_questions_json(completion_results, questions_map, output_dir, latency_runs_results=None, target_n=None):
    if target_n is None and isinstance(latency_runs_results, int):
        target_n = latency_runs_results
    ...
    for idx, new_completions in completion_results.items():
        ...
        out_file = out_dir / f"{idx}.json"
        existing = load_existing_livecodebench_output(out_file)
        existing_obj = existing["data"]
        if target_n is not None:
            needed = max(int(target_n) - len(existing["valid_completion_list"]), 0)
            new_completions = new_completions[:needed]
        ...
        for k, v in item.items():
            if k not in LIVECODEBENCH_OUTPUT_SKIP_KEYS:
                obj[k] = v
...
def livecodebench_needs_sampling(output_dir, idx, target_n):
    out_file = Path(output_dir) / f"{idx}.json"
    existing = load_existing_livecodebench_output(out_file)
    return existing["valid_completions"] < target_n
...
def main(
    ...
    input: str = "",
    input_file: str = "livecodebench_v6.jsonl",
    gpqa_file: str = "",
    output: str = "",
    save_dir = "./outputs/",
    max_turns: int = 6,
    ...
    save_dir = output or save_dir
    os.makedirs(save_dir, exist_ok=True)

    infer_pipeline = DirectVLLMInferPipeline(
        model_path,
        num_gpus,
    ...
    data_file = gpqa_file or input or input_file
    test_data, data_path = load_livecodebench_data(data_file)
    if end_idx is None:
        end_idx = len(test_data)
    end_idx = min(end_idx, len(test_data))
    assert 0 <= start_idx < end_idx <= len(test_data)
    selected_data = test_data[start_idx:end_idx]
    indexed_data = [
        (start_idx + local_idx, normalize_livecodebench_item(item, start_idx + local_idx))
        for local_idx, item in enumerate(selected_data)
    ]
    pending_indexed_data = [
        (idx, item)
        for idx, item in indexed_data
        if livecodebench_needs_sampling(save_dir, idx, n)
    ]
    skipped = len(indexed_data) - len(pending_indexed_data)
    if skipped:
        print(f"[LeaP][LiveCodeBench] skipped {skipped} completed questions in {save_dir}")

    print(f"[LeaP][LiveCodeBench] loaded {len(indexed_data)} questions from {data_path}")
    ...
    for indexed_batch in tqdm(batched_indexed_data):
        ...
        save_livecodebench_questions_json(
            completion_results,
            questions_map,
            save_dir,
            latency_runs_results,
