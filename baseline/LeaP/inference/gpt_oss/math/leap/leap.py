...
from .prompts import GPQA_temp, leap_prefix_MATH, leap_prefix_GPQA, get_leap, GPQA_answer_prompt, MATH_answer_prompt, MATH_cot, GPQA_cot
...
class LeaP:
    ...
    def _infer_texts(self, ray_pipeline, tokenizer, prompts, config):
        ...
        return texts, token_counts, finish_reasons, stop_reasons, latency
    ...
    def mixture(self, prompts, ray_pipeline, summarize_config, batch_counter, tokenizer=None, path_token_counts=None, return_metadata=False):
        ...
        if return_metadata:
            summarize_results, summarize_token_counts, _, _, _ = self._infer_texts(
                ray_pipeline,
                tokenizer,
                prompts,
                summarize_config,
    ...
    def infer_batch(self, batched_data, ray_pipeline: RayInferPipeline, tokenizer, config: GenerateConfig, summarize_config: GenerateConfig, question, return_metadata=False):
        ...
        first_sampling, first_token_counts, first_finish_reasons, first_stop_reasons, first_latency = self._infer_texts(
            ray_pipeline,
            tokenizer,
            prompts,
            sub_config,
        ...
        for turn in range(self.max_turns - 1):
            # second sampling
            sampling_results, token_counts, finish_reasons, stop_reasons, sampling_latency = self._infer_texts(
                ray_pipeline,
                tokenizer,
                prompts,
                sub_config,
        ...
        if len(prompts):
            ...
            finial_results, final_token_counts, final_finish_reasons, _, final_latency = self._infer_texts(
                ray_pipeline,
                tokenizer,
                prompts,
                summarize_config,
