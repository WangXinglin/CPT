...
GPT_OSS_STOP_TOKEN_IDS = {200002, 200012}
...
def is_stop(p, stop_token=None, finish_reason=None, stop_reason=None):
    try:
        normalized_stop_reason = int(stop_reason)
    except (TypeError, ValueError):
        normalized_stop_reason = stop_reason

    if normalized_stop_reason in GPT_OSS_STOP_TOKEN_IDS:
        return True
    if finish_reason == "stop":
        return True
    if stop_token and p[-len(stop_token):] == stop_token:
        return True
    return False
