...
def is_correct(completion, answer, verbose=False):
    ...
    if extract_ans == "{}" or extract_ans == "" or extract_ans is None:
        if is_reference_ans_number:
            numbers = re.findall(r"[\-+]?\d*[\.,/]?\d+", completion)
            if numbers: 
                extract_ans = numbers[-1]
            # print(f"2 extract_ans: {extract_ans}")
        # extract "the answer is ..." answer
        elif ("answer is" in completion) or ("solution is" in completion) or ("answer should be" in completion):
            ...
            elif "answer should be" in completion:
                split_ans = completion.split('answer should be')
    ...
    return judge, clean_prediction_ans
