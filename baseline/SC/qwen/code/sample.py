...
Dataset: LiveCodeBench code-generation JSONL.

Outputs (per question): {idx}.json
- Each file is a standard JSON file,
  storing LiveCodeBench question/meta + all completions so far (supports resume).
...
def get_lcb_question_content(item: Dict[str, Any]) -> str:
    return _text(item.get("question_content", item.get("question", "")))

def get_lcb_starter_code(item: Dict[str, Any]) -> str:
    return _text(item.get("starter_code", ""))

def get_lcb_question_id(item: Dict[str, Any], idx: int) -> Any:
    return item.get("question_id", item.get("id", f"q_{idx}"))

def format_prompt_livecodebench(question_content: str, starter_code: str = "") -> str:
    """Build the LiveCodeBench generic code-generation prompt."""
    prompt = f"### Question:\n{question_content}\n\n"
    if starter_code:
...
def save_questions_json(
    ...
    for idx, new_cs in completion_results.items():
        ...
        qid = get_lcb_question_id(item, idx)
        obj: Dict[str, Any] = {
            "index": idx,
            "question_id": qid,
            "question_content": get_lcb_question_content(item),
            "starter_code": get_lcb_starter_code(item),
            "completions": all_cs,
...
def batch_sample(
    ...
    for chunk_id in range(total_chunks):
        ...
        for q_idx, item in chunk:
            user_content = format_prompt_livecodebench(
                get_lcb_question_content(item),
                get_lcb_starter_code(item),
            )
            messages = [
                {"role": "system", "content": SYSTEM_MESSAGE_GENERIC if system_prompt is None else system_prompt},
...
def main():
    parser = argparse.ArgumentParser(description="Qwen self-consistency sampling for LiveCodeBench")
    parser.add_argument("--model", "-m", type=str, required=True, help="Model path")
    parser.add_argument("--input", "-i", type=str, required=True, help="Input LiveCodeBench JSONL")
