...
if [ "$#" -ne 2 ]; then
  echo "Usage: bash baseline/SC/qwen/math/run.sh <start_index> <end_index>"
  echo "Example: bash baseline/SC/qwen/math/run.sh 0 50"
  exit 1
fi
...
OUTPUT_ROOT="${OUTPUT_ROOT:-/path/to/your/output/sc/qwen/math}"
...
echo "Qwen math sampling completed: ${OUT_DIR}"
