...
"""
Self-consistency sampling with GPT-OSS via vLLM (no evaluation).
Datasets: AIME and HMMT competition math JSONL files.

Outputs (per question): {idx}.json
- Each file is a standard JSON file,
  storing question/meta + all completions so far (supports resume).
...
def save_questions_json(
    ...
    for idx, new_cs in completion_results.items():
        ...
        qid = item.get("question_id", item.get("id", f"q_{idx}"))
        obj: Dict[str, Any] = {
            "index": idx,
            "question_id": qid,
        ...
        for k, v in item.items():
            if k not in ["question_id", "id", "question", "answer"]:
                obj[k] = v
...
def main():
    parser = argparse.ArgumentParser(description="GPT-OSS self-consistency sampling for AIME/HMMT")
    parser.add_argument("--model", "-m", type=str, required=True, help="Model path")
    parser.add_argument("--input", "-i", type=str, required=True, help="Input JSONL")
    parser.add_argument("--output", "-o", type=str, required=True, help="Output dir")

    parser.add_argument("--n-completions", "-n", type=int, default=512)
