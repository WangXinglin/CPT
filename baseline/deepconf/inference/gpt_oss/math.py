#!/usr/bin/env python3
"""
DeepConf baseline sampler for GPT-OSS mathematical reasoning.
...
question. Each completion stores the common sampling fields together with
DeepConf confidence and extracted-answer fields, while each question result
stores all seven DeepConf voting methods. Both offline generation and online
confidence-based early stopping are supported.

Examples:
  python inference/gpt_oss/math.py --model /path/to/model --input data.jsonl --output output_dir --n-completions 64 --tensor-parallel-size 8
  python inference/gpt_oss/math.py --model /path/to/model --input data.jsonl --output output_dir --n-completions 64 --tensor-parallel-size 8 --deepconf-mode online --warmup-traces 16 --confidence-percentile 90
"""
import json
import argparse
import copy
...
# Default mathematical reasoning prompt and GPT-OSS control tokens.
DEFAULT_MATH_PROMPT = "Please reason step by step, and put your final answer within \\boxed{}."
DEFAULT_GPT_OSS_REASONING_EFFORT = "high"
GPT_OSS_RETURN_TOKEN_ID = 200002  # <|return|>
GPT_OSS_STOP_TOKEN_IDS = [GPT_OSS_RETURN_TOKEN_ID, 200012]  # <|return|>, <|call|>
...
def _to_jsonable(obj: Any) -> Any:
    """Recursively convert NumPy containers to JSON-serializable values."""
...
def process_single_output(vllm_output, window_size: int) -> Dict[str, Any]:
    ...
    stop_reason = getattr(vllm_output, "stop_reason", None)
...
def compute_voting_for_completions(completions: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    return _to_jsonable(voting_results)
    ...
    for chunk_idx in range(total_chunks):
        ...
        for original_idx, item in current_chunk:
            ...
            full_prompt = tokenizer.apply_chat_template(
                ...
                reasoning_effort=DEFAULT_GPT_OSS_REASONING_EFFORT,
            )
...
def _generate_online(
    llm, prompt: str, total_budget: int,
    temperature: float, top_p: float, top_k: int, max_tokens: int,
    window_size: int, logprobs_topk: int,
    ...
    if remaining > 0:
        ...
        for i in range(remaining):
            sp = SamplingParams(
                ...
                extra_args={
                    ...
                    "eos_token_id": GPT_OSS_RETURN_TOKEN_ID,
        ...
        for output in final_outputs:
            for out in output.outputs:
                comp = process_single_output(out, window_size)
                comp['early_stop_threshold'] = float(conf_bar)
                # vLLM does not reliably expose which stop token fired for
                # stop_token_ids, so use the same confidence condition that
                # drives the logits processor as the persisted marker.
                comp['early_stopped'] = bool(
                    comp['finish_reason'] == "stop"
                    and comp['min_conf'] < conf_bar
                )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="DeepConf baseline sampler for GPT-OSS math tasks"
    )
