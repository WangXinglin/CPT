#!/bin/bash
# ==============================================================================
# DeepConf Qwen LiveCodeBench inference script
#
# Usage:
#   bash scripts/qwen/code.sh <start_index> <end_index>
# ==============================================================================
...
if [ $# -ne 2 ]; then
    echo "错误: 参数不足"
    echo "用法: bash scripts/qwen/code.sh <start_index> <end_index>"
    echo "示例: bash scripts/qwen/code.sh 0 5"
    exit 1
fi
...
OUTPUT_BASE_DIR="${OUTPUT_BASE_DIR:-/path/to/your/output/deepconf/qwen/code}"
...
mkdir -p "$OUTPUT_DIR"
INFER_CMD=(
    python "$DEEPCONF_ROOT/inference/qwen/code.py"
    --model "$MODEL"
    --input "$INPUT"
    --output "$OUTPUT_DIR"
