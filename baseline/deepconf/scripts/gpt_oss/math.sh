#!/bin/bash
# ==============================================================================
# DeepConf GPT-OSS math inference script
#
# Usage:
#   bash scripts/gpt_oss/math.sh <start_index> <end_index>
# ==============================================================================
...
if [ $# -ne 2 ]; then
    echo "错误: 参数不足"
    echo "用法: bash scripts/gpt_oss/math.sh <start_index> <end_index>"
    echo "示例: bash scripts/gpt_oss/math.sh 0 5"
    exit 1
fi
...
MODEL="${MODEL:-/path/to/your/gpt-oss-model}"
DATA_BASE_DIR="${DATA_BASE_DIR:-/path/to/your/data}"
OUTPUT_BASE_DIR="${OUTPUT_BASE_DIR:-/path/to/your/output/deepconf/gpt_oss/math}"
...
OUTPUT_DIR="${OUTPUT_BASE_DIR}/deepconf_${SHORT_NAME}_gpt20b_N${N_COMPLETIONS}"

# ==============================================================================

echo "============================================================"
echo "  DeepConf gpt20b 推理"
echo "============================================================"
echo "  模型:             $MODEL"
...
if [ "$INSTALL_REQUIREMENTS" = "1" ]; then
    ...
    pip install -r "$DEEPCONF_ROOT/requirements-gpt-oss.txt"
    echo ""
fi

mkdir -p "$OUTPUT_DIR"
INFER_CMD=(
    python "$DEEPCONF_ROOT/inference/gpt_oss/math.py"
    --model "$MODEL"
    --input "$INPUT"
    --output "$OUTPUT_DIR"
