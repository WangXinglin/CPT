...
Usage:
  ...
  bash scripts/gpt_oss/math.sh <num_splits> <split_id>
...
PROGRAM="${REPO_ROOT}/code/gpt_oss/math.py"
...
OUTPUT_ROOT="${OUTPUT_ROOT:-${REPO_ROOT}/outputs/gpt_oss/math}"
...
CHUNK_TOKENS_FIXED="${CHUNK_TOKENS_FIXED:-3072}"
...
OUTPUT_DIR="${OUTPUT_ROOT}/gpt_oss_math_${DATASET}_N${N_COMPLETIONS}"
...
mkdir -p "${OUTPUT_DIR}"
echo "Running gpt_oss/math: dataset=${DATASET}, N=${N_COMPLETIONS}"
"${PYTHON_BIN}" "${PROGRAM}" \
    --model "${MODEL_PATH}" \
    --input "${INPUT_FILE}" \
